...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for xy in range(2):
        ...
        for i in range(0,len(data)):
            ...
            for j in range(0, len(dataL)):
                l = N+dataL[j][0]
                xiX.value[l][0]+=dataL[j][xy+1]/measurement_noise
                xiX.value[i][0]-=dataL[j][xy+1]/measurement_noise
                omegaX.value[l][l]+=1.0/measurement_noise
                omegaX.value[i][i]+=1.0/measurement_noise
                omegaX.value[l][i]-=1.0/measurement_noise
                omegaX.value[i][l]-=1.0/measurement_noise

            n = i+1
            xiX.value[n][0] += dataG[xy]/motion_noise
            xiX.value[i][0] -= dataG[xy]/motion_noise
            omegaX.value[n][n] += 1.0/motion_noise
            omegaX.value[i][i] += 1.0/motion_noise
            omegaX.value[n][i] -= 1.0/motion_noise
            omegaX.value[i][n] -= 1.0/motion_noise

        #
        # for x in range(omegaX.dimx):
        #     print omegaX.value[x]
        #
        # print "xiX," , xiX

        omegaInv=omegaX.inverse()
        a = omegaInv * xiX
        coll.append(a)
    #
    # print coll[0]
    mu=matrix()
    mu.zero(50,1)
    for xy in range(len(coll)):
        for i in range(coll[0].dimx):
            mu.value[2*i+xy]=coll[xy].value[i]
