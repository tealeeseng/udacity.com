...
class matrix:
    ...
    def __mul__(self, other):
        ...
        if self.dimy != other.dimx:
            raise ValueError, "Matrices must be m*n and n*p to multiply. self.dimy="+str(self.dimy)+" and other.dimx="+str(other.dimx)
        else:
...
# def scriptTest(num_landmarks):
#     expRange = [x for x in range(2 * (1 + num_landmarks))]
#     expRange.remove(2)
#     expRange.remove(3)
#     print expRange
#
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # if k<>0:
            #expand.
        expRange = [x for x in range(2*(2+num_landmarks))]
        expRange.remove(2)
        expRange.remove(3)

        Omega = Omega.expand(Omega.dimx+2, Omega.dimy+2, expRange, expRange)
        Xi = Xi.expand(Xi.dimx+2, Xi.dimy,expRange,[0])
        ...
        # if k<>0:
            #reduce
        reduceRange = [x for x in range(2*(2+num_landmarks))]
        reduceRange.remove(0)
        reduceRange.remove(1)
        OmegaPi = Omega.take(reduceRange, reduceRange)
        XiPi=Xi.take(reduceRange,[0])
        A = Omega.take([0,1],reduceRange)
        B = Omega.take([0,1],[0,1])
        C = Xi.take([0,1],[0])
        Omega = OmegaPi - A.transpose()*B.inverse()*A
        Xi = XiPi - A.transpose()*B.inverse()*C
