...
class robot:
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        # print distance, self.length, tan(alpha), beta
        x = 0
        y = 0

        if (abs(beta) > 0.001):
            r = distance / beta
            x = self.x - sin(self.orientation) * r + sin(self.orientation + beta) * r
            y = self.y + cos(self.orientation) * r - cos(self.orientation + beta) * r
        else:
