...
def run(grid, goal, spath, params, printflag=False, speed=0.1, timeout=1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        #     if (spath[i][0]<=estimate[0]<=spath[i+1][0] or\
        #         spath[i][0]>=estimate[0]>=spath[i+1][0]) and\
        #             (spath[i][1]<=estimate[1]<=spath[i+1][1] or \
        #                 spath[i][1] >= estimate[1] >= spath[i + 1][1]):
